...
from app.schemas.payment import CreditPackage, PaymentInitRequest, PaymentInitResponse
...
async def payment_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    # Verify signature
    expected_signature = hmac.new(settings.FLUTTERWAVE_WEBHOOK_SECRET.encode(), body, hashlib.sha256).hexdigest()
...
async def handle_successful_payment(data: dict, db: Session):
    ...
    # Find transaction
    transaction = db.query(CreditTransaction).filter(CreditTransaction.flutterwave_tx_ref == tx_ref).first()

    if not transaction:
        logger.error("Transaction not found for tx_ref: %s", tx_ref)
        return

    if transaction.status == TransactionStatus.COMPLETED:
        logger.info("Transaction %s already processed", tx_ref)
        return
    ...
    db.commit()
    logger.info("Successfully processed payment for tx_ref: %s", tx_ref)
...
async def handle_failed_payment(data: dict, db: Session):
    ...
    if transaction and transaction.status == TransactionStatus.PENDING:
        ...
        db.commit()
        logger.info("Marked transaction as failed for tx_ref: %s", tx_ref)
...
@router.get("/balance")
async def get_credit_balance(user: User = Depends(get_current_user_required), db: Session = Depends(get_db)):
