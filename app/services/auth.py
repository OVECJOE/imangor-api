...
class AuthService:
    ...
    def get_or_create_user(self, google_info: Dict) -> User:
        ...
        user = self.db.query(User).filter(User.google_id == google_info["google_id"]).first()
        ...
        credit_service.add_credits(str(user.id), settings.FREE_CREDITS_NEW_USER, "Welcome bonus for new user")
